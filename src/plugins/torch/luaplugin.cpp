...
void LuaPlugin::writeMap(LuaTableWriter &writer, const Map *map)
{
    ...
    writer.writeKeyAndValue("tile_width", map->tileWidth());
    writer.writeKeyAndValue("tile_height", map->tileHeight());
                             ...
                             unsigned firstGid)
{
     ...
     */
    writer.writeKeyAndValue("tile_width", tileset->tileWidth());
    writer.writeKeyAndValue("tile_height", tileset->tileHeight());
    ...
    if (!tileset->imageSource().isEmpty()) {
		...
		QString &name = filename.remove(filename.indexOf('.'), filename.length());
        writer.writeKeyAndValue("name", name);
	...
	*/
    ...
    writer.writeStartTable("tile_offset");
    ...
    for (int i = 0; i < tileset->tileCount(); ++i) {
        ...
        if (tile->isAnimated()) {
            ...
            foreach (const Frame &frame, frames) {
                writer.writeStartTable();
                writer.writeKeyAndValue("id", QString::number(frame.tileId));
...
Q_EXPORT_PLUGIN2(Torch, LuaPlugin)
#endif
